...
install: ## Install composer deps
	...
	$(docker_compose_bin) -f docker-compose.yml exec -T --workdir="${SITE_APP_DIR}"/skeleton --user="${CURRENT_USER}:${CURRENT_USER_ID}" ${APACHE_CONTAINER_NAME} find .env -type f -exec sed -i "s${DATABASE_DEFAULT}/${DATABASE_URL}/g" {} \;
